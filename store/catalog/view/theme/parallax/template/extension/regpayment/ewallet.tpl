...
$('#button-confirm').on('click', function() {
        $.ajax({
            type: 'post',
            url: 'index.php?route=extension/regpayment/ewallet/confirm/',
            data: $('#payment :input'),
            dataType: 'json',
            cache: false,
            beforeSend: function() {
    ...
    $("#username").blur(function()
    {
        if ($("#username").val() != '')
        {
            var user_name_availability = $('base').attr('href') + "index.php?route=extension/regpayment/ewallet/validateUsername";
            var msg;
