...
class ControllerExtensionRegPaymentStripe extends Controller {
	...
	public function confirm(){
        ...
        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
        $pack_info =$this->model_checkout_order->getOrderProduct($this->session->data['order_id']);
        ...
        if (!isset($json['redirect'])) {
            ...
            $first_name='';
            $last_name='';
            if(isset($step4_data['firstname'])){
                $first_name = $step4_data['firstname'];
            }if(isset($step4_data['lastname'])){
                $last_name = $step4_data['lastname'];
            }
            $email=$step4_data['email'];
            ...
            // Charge the order:
                    /*            try {
                        ...
                        $this->model_checkout_order->addStripResponse(serialize($curl_data), 'repurchase_strip token create order_id:' . $this->session->data['order_id'], json_encode(substr($ex, 0, 1000)));
...
//echo "<pre>"."--".$this->request->post['stripeToken'];print_r($ex);die;
                        $this->model_checkout_order->addStripResponse(serialize($curl_data), 'repurchase_strip customer create order_id:' . $this->session->data['order_id'], json_encode(substr($ex, 0, 1000)));

                        $json['error'] = 'Payment unsuccessfull.Please verify the details.';
                         $error = $ex->getMessage();
                        $this->session->data['error']='There was a problem charging your card:'.$error;
                        $this->log->write('Payment unsuccessfull.Please verify the details.');
                        //$json['redirect'] = $this->url->link('checkout/checkout', '', false);
                    }
                    if(isset($customer->id)){
                        $this->session->data['stripe_customer_id']=$customer->id;
                    }else{
                        if(!isset($this->session->data['error'])){
                            $this->session->data['error']="There was a problem charging your card: Card Declined";
                        }
                    }
            ...
            try {
                $charge = \Stripe\Charge::create([
                            ...
                            "description" => "Registration order Id:" . $order_info['order_id']." with ".$pack_name,
...
//
               // foreach ($recurring_products as $item) {
                    ...
                    if(isset($this->session->data['stripe_customer_id'])) {
                   $recurr_tran_id = $this->model_checkout_order->addSubscriptionDetails(serialize($curl_data), 'Repuerchase_reccuring_subcription_fadded order_id:' . $this->session->data['order_id'], $this->session->data['stripe_customer_id'], $this->session->data['order_id'], $item, json_encode($customer));
                    }
                //}
