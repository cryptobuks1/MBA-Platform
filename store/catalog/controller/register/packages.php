...
class ControllerRegisterPackages extends Controller {
    ...
    public function index() {
        ...
        if (isset($this->request->get['action'])) {
            if ($this->request->get['action'] == "restart") {
                $this->cart->clear();
            }
        }
        ...
        if ($from != 'reg') {
            $data['continue'] = $this->url->link('register/user', '', true);
        } else {
            $data['continue'] = $this->url->link('register/user', 'step=3', true);
        }
        ...
        $save_value = array(
            ...
            'wg24themeoptionpanel_c_sub_categor_prallax'
        );
        ...
        $data['button_continue'] = $this->language->get('button_continue');
        $data['text_registration_package'] = $this->language->get('text_registration_package');
        ...
        $filter_data = array(
            'filter_package_type' => 'registration',
...
//echo "<pre>";print_r($results);die;
        ...
        $data['sorts'][] = array(
            ...
            'href' => $this->url->link('register/packages', 'sort=p.sort_order&order=ASC' . $url)
        );

        $data['sorts'][] = array(
            'text' => $this->language->get('text_name_asc'),
            'value' => 'pd.name-ASC',
            'href' => $this->url->link('register/packages', 'sort=pd.name&order=ASC' . $url)
        );

        $data['sorts'][] = array(
            'text' => $this->language->get('text_name_desc'),
            'value' => 'pd.name-DESC',
            'href' => $this->url->link('register/packages', 'sort=pd.name&order=DESC' . $url)
        );

        $data['sorts'][] = array(
            'text' => $this->language->get('text_price_asc'),
            'value' => 'p.price-ASC',
            'href' => $this->url->link('register/packages', 'sort=p.price&order=ASC' . $url)
        );

        $data['sorts'][] = array(
            'text' => $this->language->get('text_price_desc'),
            'value' => 'p.price-DESC',
            'href' => $this->url->link('register/packages', 'sort=p.price&order=DESC' . $url)
        );
        ...
        if ($this->config->get('config_review_status')) {
            $data['sorts'][] = array(
                ...
                'value' => 'rating-DESC',
                'href' => $this->url->link('register/packages', 'sort=rating&order=DESC' . $url)
            );

            $data['sorts'][] = array(
                'text' => $this->language->get('text_rating_asc'),
                'value' => 'rating-ASC',
                'href' => $this->url->link('register/packages', 'sort=rating&order=ASC' . $url)
            );
        }
        ...
        $data['sorts'][] = array(
            ...
            'value' => 'p.model-ASC',
            'href' => $this->url->link('register/packages', 'sort=p.model&order=ASC' . $url)
        );

        $data['sorts'][] = array(
            'text' => $this->language->get('text_model_desc'),
            'value' => 'p.model-DESC',
            'href' => $this->url->link('register/packages', 'sort=p.model&order=DESC' . $url)
        );
        ...
        foreach ($limits as $value) {
            $data['limits'][] = array(
                'text' => $value,
                'value' => $value,
                'href' => $this->url->link('register/packages', $url . '&limit=' . $value)
            );
        }
        ...
        $pagination->url = $this->url->link('register/packages', $url . '&page={page}');
        ...
        if ($page == 1) {
            $this->document->addLink($this->url->link('register/packages', true), 'canonical');
        } elseif ($page == 2) {
            $this->document->addLink($this->url->link('register/packages', true), 'prev');
        } else {
            $this->document->addLink($this->url->link('register/packages', 'page=' . ($page - 1), true), 'prev');
        }

        if ($limit && ceil($product_total / $limit) > $page) {
            $this->document->addLink($this->url->link('register/packages', 'page=' . ($page + 1), true), 'next');
        }
        ...
        $this->response->setOutput($this->load->view('register/packages', $data));
    }
...
//        $this->document->addScript('catalog/view/javascript/cart_register.js');
        ...
        $data['button_wishlist'] = $this->language->get('text_wishlist');
        $data['button_compare'] = $this->language->get('text_compare');
