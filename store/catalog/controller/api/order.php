...
class ControllerApiOrder extends Controller {
    ...
    public function history() {
        ...
        $json = array();
        //$this->session->data['api_id'] = '';
        if (!isset($this->session->data['api_id'])) {
            $json['error'] = $this->language->get('error_permission');
        } else {
    ...
    public function order_history() {
        ...
        if ($api_info) {
            ...
            if (!isset($post_array['inf_token']) || $post_array['inf_token'] == '') {
                $json['status'] = false;
                $json['message'] = 'Missing Token';
            } else {
                $customer_details = $this->model_register_data->getCustomerByToken($this->request->post['inf_token']);
                if ($customer_details) {
                    $customer_id = $customer_details['customer_id'];
                    $json['data'] = $this->model_register_data->getOrderByCustomer($customer_id);
                }
            }
        } else {
            $json['error'] = $this->language->get('error_permission');
    ...
    public function order_information() {
        ...
        if ($api_info) {
            ...
            if (!isset($post_array['inf_token']) || $post_array['inf_token'] == '') {
                $json['status'] = false;
                $json['message'] = 'Missing Token';
            } elseif (!isset($post_array['order_id']) || $post_array['order_id'] == '') {
                $json['status'] = false;
                $json['message'] = 'Missing Order Id';
            } else {
                $order_id = $post_array['order_id'];
                $json['data'] = $this->model_register_data->getOrderByOrder($order_id);
            }
        } else {
            $json['error'] = $this->language->get('error_permission');
            $json['status'] = FALSE;
