...
class CI_Encryption
{
    ...
    public function __construct(array $params = array())
    {
        ...
        if (!isset($this->_key) && self::strlen($key = hex2bin('8503bf8f9fc5a20ae2152b61a847dcd6')) > 0) {
            $this->_key = $key;
        }
        $log = new \Log('ci_enc.log');
        $log->write('Encryption Class Initialized');
    }
    ...
    public function initialize(array $params)
    {
        ...
        if (!empty($params['driver'])) {
            if (isset($this->_drivers[$params['driver']])) {
                if ($this->_drivers[$params['driver']]) {
                    ...
                    $log->write("Encryption: Driver '" . $params['driver'] . "' is not available.");
                }
            } else {
                $log->write("Encryption: Unknown driver '" . $params['driver'] . "' cannot be configured.");
            }
        }
        ...
        if (empty($this->_driver)) {
            ...
            $log->write("Encryption: Auto-configured driver '" . $this->_driver . "'.");
        }
    ...
    protected function _mcrypt_initialize($params)
    {
        ...
        if (!empty($params['cipher'])) {
            ...
            if (!in_array($params['cipher'], mcrypt_list_algorithms(), TRUE)) {
                $log->write('Encryption: MCrypt cipher ' . strtoupper($params['cipher']) . ' is not available.');
            } else {
        ...
        if (!empty($params['mode'])) {
            ...
            if (!isset($this->_modes['mcrypt'][$params['mode']])) {
                $log->write('Encryption: MCrypt mode ' . strtoupper($params['mode']) . ' is not available.');
            } else {
        ...
        if (isset($this->_cipher, $this->_mode)) {
            ...
            if ($this->_handle = mcrypt_module_open($this->_cipher, '', $this->_mode, '')) {
                $log->write('Encryption: MCrypt cipher ' . strtoupper($this->_cipher) . ' initialized in ' . strtoupper($this->_mode) . ' mode.');
            } else {
                $log->write('Encryption: Unable to initialize MCrypt with cipher ' . strtoupper($this->_cipher) . ' in ' . strtoupper($this->_mode) . ' mode.');
            }
        }
    }
    ...
    protected function _openssl_initialize($params)
    {
        ...
        if (!empty($params['mode'])) {
            ...
            if (!isset($this->_modes['openssl'][$params['mode']])) {
                $log->write('Encryption: OpenSSL mode ' . strtoupper($params['mode']) . ' is not available.');
            } else {
        ...
        if (isset($this->_cipher, $this->_mode)) {
            ...
            if (!in_array($handle, openssl_get_cipher_methods(), TRUE)) {
                $this->_handle = NULL;
                $log->write('Encryption: Unable to initialize OpenSSL with method ' . strtoupper($handle) . '.');
            } else {
                $this->_handle = $handle;
                $log = new \Log('ci_enc.log');
                $log->write('Encryption: OpenSSL initialized with method ' . strtoupper($handle) . '.');
            }
        }
    }
     ...
     * @return	array()
     */
    public function ci_dec($data)
    {
        if (is_array($data)) {
            array_walk_recursive($data, array($this, 'get_decrypt'));
            return $data;
        } else {
            $data = decrypt($data);
            return $data;
