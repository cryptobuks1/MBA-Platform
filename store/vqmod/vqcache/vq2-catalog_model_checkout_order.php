...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$order_status_id . "',view_status = '1',date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
	 ...
	 public function getRegAmount(){
            $query = $this->db->query("SELECT reg_amount FROM " . MLM_DB_PREFIX . "configuration");

		return $query->row['reg_amount'];
        }
        
        
   public function checkOrderAlreadyConfirmed($order_id) {
        $query = $this->db->query("SELECT order_history_id FROM `" . DB_PREFIX . "order_history` WHERE order_id = '" . $order_id . "' and order_status_id = 5");
        ...
        $query = $this->db->query("SELECT stripe_key FROM " . MLM_DB_PREFIX . "configuration WHERE id = 1");

        if ($query->num_rows) {
            return $query->row["stripe_key"];
        }
    }
    public function get_stripe_secret_key() {
        $query = $this->db->query("SELECT stripe_secret FROM " . MLM_DB_PREFIX . "configuration WHERE id = 1");

        if ($query->num_rows) {
            return $query->row["stripe_secret"];
        }
    }

    public function addStripResponse($curl_data, $curl_type, $curl_result) {
        // print_r("INSERT INTO " . DB_PREFIX . "strip_curl_history SET curl_url = 'stripe', curl_type = '" . $curl_type . "', curl_data = '" . $curl_data . "', curl_date = '" . date('Y-m-d H:i:s') . "', curl_result = " . $curl_result . "");

        $this->db->query("INSERT INTO " . DB_PREFIX . "strip_curl_history SET curl_url = 'stripe', curl_type = '" . $curl_type . "', curl_data = '" . $curl_data . "', curl_date = '" . date('Y-m-d H:i:s') . "', curl_result = " . $curl_result . "");
    }

    public function addSubscriptionDetails($curl_data, $curl_type, $recurring_id, $order_id, $item, $subscription) {

        $this->db->query("INSERT INTO `" . DB_PREFIX . "order_recurring` SET `stripe_customer_id` = '" . $recurring_id . "',`order_id` = '" . (int) $order_id . "', `date_added` = NOW(), `status` = 6, `product_id` = '" . (int) $item['product_id'] . "', `product_name` = '" . $this->db->escape($item['name']) . "', `product_quantity` = '" . $this->db->escape($item['quantity']) . "', `recurring_id` = '" . (int) $item['recurring']['recurring_id'] . "', `recurring_name` = '" . $this->db->escape($item['recurring']['name']) . "', `recurring_description` = 'subscription_added', `recurring_frequency` = '" . $this->db->escape($item['recurring']['frequency']) . "', `recurring_cycle` = '" . (int) $item['recurring']['cycle'] . "', `recurring_duration` = '" . (int) $item['recurring']['duration'] . "', `recurring_price` = '" . (float) $item['recurring']['price'] . "', `trial` = '" . (int) $item['recurring']['trial'] . "', `trial_frequency` = '" . $this->db->escape($item['recurring']['trial_frequency']) . "', `trial_cycle` = '" . (int) $item['recurring']['trial_cycle'] . "', `trial_duration` = '" . (int) $item['recurring']['trial_duration'] . "', `trial_price` = '" . (float) $item['recurring']['trial_price'] . "', `reference` = '" . $subscription . "'");
        //print_r($item['recurring']['frequency']);die;

        $today = strtotime(date("Y-m-d"));
        $nxt_reccuring = date('Y-m-d', strtotime('+30 days', $today));
        $this->db->query("INSERT INTO `" . DB_PREFIX . "order_recurring_transaction` SET `order_recurring_id` = '" . $this->db->getLastId() . "',`reference` = '" . $recurring_id . "',`nxt_reccuring_date` = '" . $nxt_reccuring . "', `date_added` = NOW(), `type` = 'reccuring_added', `amount` = '" . $this->db->escape($item['quantity']) * $item['recurring']['price'] . "'");
        return $this->db->getLastId();
    }
    public function addNbaSubscriptionDetails($recurring_id, $token, $item) {
        $today = strtotime(date("Y-m-d"));
        $nxt_reccuring = date('Y-m-d', strtotime('+30 days', $today));
        $this->db->query("INSERT INTO `" . DB_PREFIX . "order_recurring_transaction` SET `order_recurring_id` = '" . $recurring_id . "',`reference` = '" . $token . "',`nxt_reccuring_date` = '" . $nxt_reccuring . "', `date_added` = NOW(), `type` = 'reccuring_added', `amount` = '" . $this->db->escape($item['quantity']) * $item['recurring']['price'] . "'");
    }
    public function addNbaResponse($data, $order_id, $type, $result) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "nba_txn_history SET order_id = '" . $order_id . "' , txn_url = 'nba', txn_type = '" . $type . "', txn_data = '" . $data . "', date = '" . date('Y-m-d H:i:s') . "', txn_result = '" . $result . "'");
    }
    public function addNbaError($data, $order_id, $type, $result) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "nba_txn_error_history SET order_id = '" . $order_id . "' , txn_url = 'nba', txn_type = '" . $type . "', txn_data = '" . $data . "', date = '" . date('Y-m-d H:i:s') . "', txn_result = '" . $result . "'");
    }
    public function updateRecurringCustID($cust_id,$tran_id) {
        $q="UPDATE `" . DB_PREFIX . "order_recurring_transaction` SET oc_cust_id = '" . (int)$cust_id . "' WHERE order_recurring_transaction_id = '" . (int)$tran_id . "'";
        	$this->db->query($q);
    }
