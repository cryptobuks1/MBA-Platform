...
//print_r($active_tab);die;
    ...
    public function squareup_payment()
    {
      ...
      //  die('Nopp');
      $user_array =  $this->order_model->getPendingRegDetails(0, 0, 0, 1); 
      print_r($user_array);die;
       foreach($user_array as $user){
            
           $user['sponsor_user_name']= $this->validation_model->getUserNameFromMigId($user['Referal_IDNo']);
           $user['placement_user_name']= $this->validation_model->getUserNameFromMigId($user['Upline_IDNo']);//echo "<pre>";print_r($user);
           if( $user['sponsor_user_name'] && $user['placement_user_name']){  echo "jj";
           $user['active_tab']='free_join_tab';
           $user['reg_from_tree']=0;
           $user['user_name_entry']=$user['Name'];
           $user['mobile']=$user['telephone']=$user['MobileNo'];
           $user['address']=$user['Address'];
           $user['pin']=$user['Pincode'];
           $user['nominee']=$user['Nominee'];
           $user['product_id']=28;
           $user['pswd']=$user['pswd'];
           $user['first_name']=$user['firstname']='firstname';
           $user['last_name']=$user['lastname']='lastname';
           $user['email']='test@demo.in';
           $user['country']=99;
           $user['state']=1490;
           $user['city']='calicut';
           $user['address_line2']='';
           $user['gender']='M';
         //  $user['package_validity']=28;
           $user['position']=$user['Left_side']?'L':'R';
           // Add to customer
           $user['customer_group_id'] =1;
           $user['config_store_id'] =0;
           $user['config_language_id'] =1;
           $user['fax'] =1;
           $user['newsletter'] ='';
           $user['forwarded_ip'] = '';
           $user['user_agent'] = '';
           ...
           $customer_id = $this->order_model->addCustomer($user);//echo "<pre>";print_r($user);echo $customer_id;
           $user['customer_id']=$customer_id; 
           $this->migration_submit($user);
           $update_status = $this->validation_model->updateMigStatus($user['MCNo']);
           }else{
               echo "<br>".$user['MCNo'];
           }
           }
      ...
      function migration_submit($reg_post_array){ echo "mig started ";
        if ($this->BLOCK_REGISTER && $this->LOG_USER_TYPE != 'admin' && $this->LOG_USER_TYPE != 'employee') {
            $this->redirect(lang('you_cant_access_page_due_to_block_status'), 'home', false);
        }
        $signup_settings = $this->configuration_model->getGeneralSignupConfig();
        ...
        if ($signup_settings['sponsor_required'] == 'no' && $this->LOG_USER_TYPE != 'admin' && $this->LOG_USER_TYPE != 'employee') {
            $reg_post_array['sponsor_user_name'] = $this->ADMIN_USER_NAME;
            $reg_post_array['sponsor_full_name'] = $this->validation_model->getFullName($this->ADMIN_USER_ID);
        }
        ...
        //$this->validate_register_submit()
        if (1) {
            if ($this->input->post('active_tab') == 'bank_transfer' && !isset($this->session->userdata['file'])) {
                $msg = $this->lang->line('upload_reciepts..');
                $this->redirect($msg, "register/user_register", false);
            }
          ...
          // $product_status = 'no';
            ...
            $regr['user_name_type'] ='dynamic';
            $regr['joining_date'] = date('Y-m-d H:i:s');
            $regr['active_tab'] = $active_tab;
            $regr['reg_from_tree'] = true;
            ...
            if ($is_pin_ok) {
                ...
                $this->session->set_userdata('inf_regr', $regr);
                $msg = "";
                $this->redirect($msg, "register/bitgo_gateway/", false);
            } elseif ($is_bitcoin_ok) {
                $regr['by_using'] = 'bitcoin';
                $this->session->set_userdata('inf_regr', $regr);
                $msg = "";
                $this->redirect($msg, "register/bitcoin_payment/", false);
            } elseif ($is_payeer_ok) {
                $regr['by_using'] = 'payeer';
                $data = array(
                    'user_id' => $this->LOG_USER_ID,
                ...
                $msg = "";
                $this->session->set_userdata('payeer_data', $data);
                $this->redirect($msg, "register/payeer/", false);
            } elseif ($is_sofort_ok) {
                $regr['by_using'] = 'sofort';
                $this->session->set_userdata('inf_regr', $regr);
                $msg = "";
                $this->redirect($msg, "register/sofort_payment/", false);
            }
            elseif ($is_bank_transfer_ok) {
                ...
                $regr['by_using'] = 'free join';
                $this->register_model->begin();
                $status = $this->register_model->confirmMigration($regr, $module_status, $pending_signup_status);
                if ($status['status']) {
                    $payment_status = true;
                }
            }
            ...
            if ($payment_status) {
               ...
               /* if ($product_status == "yes") {
                ...
                $this->validation_model->updateMigrateId($regr['IDNo'],$user_id);
return 1;
                //$this->redirect($msg, "register/preview/{$user_name_encrypt}", true);
            } else {
                $this->register_model->rollback();
...
(
    ...
    'age_limit' => 18,
    'mlm_plan' => 'Matrix',
    'path' => 'http://demo6.iossmlm.com/jason/backoffice_admin/',
    'lang_id' => 1,
    'path_temp' => 'http://demo6.iossmlm.com/jason/backoffice_admin/public_html/',
    'path_root' => 'http://demo6.iossmlm.com/jason/backoffice_admin/',
    'reg_from_tree' => 0,
    ...
    'registration_fee' => 0,
    'product_amount' => 100.00000000,
    'total_reg_amount' => 100,
    'total_reg_amount1' => 100.00,
    'product_status' => 'yes',
    'date_of_birt' => '',
    'default_country' => 99,
    'sponsor_user_name' => 'admin',
    'sponsor_full_name' => 'tester',
    'placement_user_name' => 'admin',
    'placement_full_name' => 'tester',
    'first_name' => 'tester',
    'last_name' => 'tester',
...
for($index = 1001;$index <= 1010;$index++){
    $data['user_name_entry'] = 'tester'.$index;
                $this->load->model('order_model');
               $customer_id = $this->order_model->addCustomer($data);
               $data['customer_id']=$customer_id;
    $res = $this->register_model->confirmRegister($data,$module_status);
    if(isset($res['status'])){
        if($res['status'] == 1){
    ...
    function addorder(){
        for($index = 1001;$index <= 1010;$index++){
            $this->order_model->addorder($index);
        }
    }
    function updatepswd(){
        $this->register_model->updatePswd();
    }
     function updateOrder(){
        for($index = 1001;$index <= 1010;$index++){
            $user_name = 'tester'.$index;
            $user_id = $this->validation_model->getUserID($user_name);
            $cust_id=$this->validation_model->getOcCustomerId($user_id);
            $this->order_model->updateOrder($index,$cust_id);
        }
    }
}
