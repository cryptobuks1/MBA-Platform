{extends file=$BASE_TEMPLATE} {block name=$CONTENT_BLOCK}{assign var="report_name" value="{lang('user_payout_report')}"} {assign var="excel_url" value="{$BASE_URL}admin/excel/create_excel_weekly_payout_report"} {assign var="csv_url" value="{$BASE_URL}admin/excel/create_csv_weekly_payout_report"}
    ...
    <div class=" table-responsive">
        <table st-table="rowCollectionBasic" class="table table-bordered table-striped">{if $count >= 1}
            <tbody>
                ...
                {assign var="i" value=0} {foreach from=$weekly_payout item=v} {$i = $i+1}
                <tr>
                    <td>{$i}</td>
                    <td>{$v.full_name}</td>
                    <td>{$v.user_name}</td>
