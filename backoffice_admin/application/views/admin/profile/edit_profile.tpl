...
{block name=$CONTENT_BLOCK}
    <div class="panel panel-default">
        <div class="panel-body">
            {form_open_multipart('admin/profile/update_profileimg_banner','role="form" class="" name= "edit_profile"  id="edit_profile"')}
            <div class="col-sm-3 padding_both">
                <div class="form-group">
                    <label class="control-label pro-label">{lang('upload_banner')}
                    </label> 
                    <div data-provides="fileupload" class="bg_file_upload pro_file_upload">
                        <input name="file2" id="file2" type="file">
                        <label id="fileLabel1">{lang('select_file')}
                        </label>
                         
                    </div>
            {lang('choose_png_jpeg_jpg')}<span class="symbol required"></span> <br>          
            {lang('Size_of_file_should_be_less_than_2mb')}<span class="symbol required"></span> 
