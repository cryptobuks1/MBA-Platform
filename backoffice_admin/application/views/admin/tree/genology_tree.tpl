...
{block name=$CONTENT_BLOCK}
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            {form_open('admin/genology_tree','role="form"  name="search_member" id="search_member" method="post"')}
            ...
            <div class="button_back m-t-sm">    
                {if $MODULE_STATUS['mlm_plan'] == "Unilevel"}
                    {if isset($MODULE_STATUS['sponsor_commission_status']) && $MODULE_STATUS['sponsor_commission_status'] == 'yes'}
                        <a href="{BASE_URL}/admin/unilevel_history" class="btn m-b-xs m-t-md btn-sm btn-info btn-addon"><i class="fa fa-forward"></i>{lang('go_to_unilevel_list')}</a>
                    {/if}
                {else}
                    <a href="{BASE_URL}/admin/binary_history" class="btn m-b-xs m-t-md btn-sm btn-info btn-addon"><i class="fa fa-forward"></i>{lang('downline_list')}</a>
                {/if}
            </div>
        </div>
    </div>
    ...
    <input id="tree_url" value="{$BASE_URL}admin/tree/tree_view" type="hidden">
{/block}
