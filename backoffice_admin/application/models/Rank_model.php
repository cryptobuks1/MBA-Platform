...
//For repurchase & upgrade ends
    ...
    public function checkQualifiedRank($personal_pv, $left_quali_pv,$right_quali_pv, $spon_count) {
        $rank_id = 0;
        $this->db->select('rank_id');
        $this->db->from('rank_details');
        $this->db->where('personal_pv <=', $personal_pv);
        $this->db->where('gpv <=', $left_quali_pv);
        $this->db->where('gpv <=', $right_quali_pv);
        $this->db->where('referal_count <=', $spon_count);
        $this->db->where('rank_status', 'active');
        $this->db->where('delete_status', 'yes');
    ...
    public function calculateRankIncentive($user_id, $new_rank) {
        $rank_bonus = $this->configuration_model->getActiveRankDetails($new_rank);
        $amount_type = "rank_incentive";
        $obj_arr = $this->validation_model->getSettings();
        $tds_db = $obj_arr["tds"];
        $date_of_sub = date("Y-m-d H:i:s");
        $service_charge = $obj_arr["service_charge"];
        $rank_amount = $rank_bonus[0]['rank_incentive_bonus'];
        $tds_amount = ($rank_amount * $tds_db) / 100;
        $service_charge = ($rank_amount * $service_charge) / 100;
        $amount_payable = $rank_amount - ($tds_amount + $service_charge);
        $this->calculation_model->insertRankIncentive($user_id, $rank_amount, $amount_payable, $tds_amount, $service_charge, $date_of_sub, $amount_type, '', '', $new_rank);
        if ($rank_bonus[0]['rank_incentive_reward'] != '') {
            $this->insertIntoUserRewards($user_id, $new_rank, $rank_bonus[0]['rank_incentive_reward'], $date_of_sub);
        }
        return true;
    }

    public function insertIntoUserRewards($user_id, $new_rank, $reward, $date_of_sub) {
        $this->db->set('user_id', $user_id);
        $this->db->set('rank_id', $new_rank);
        $this->db->set('reward', $reward);
        $this->db->set('date', $date_of_sub);
        $res = $this->db->insert('user_rewards');
        return $res;
    }
