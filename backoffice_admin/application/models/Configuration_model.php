...
class configuration_model extends inf_model
{
    ...
    public function updateAdditionalSettings($post_array, $module_status, $default_currency_value)
    {
        ...
        $settings_array['monthly_subs_fee'] = $post_array['subs_fee'];
        $settings_array['subs_referal_count'] = $post_array['subs_referal_count'];    
        $settings_array['upgrade_amount'] = $post_array['upgrade_fee']; 
        //insert configuration_change_history
        ...
        $additional_history .= "Upgrade fee: " . $this->DEFAULT_SYMBOL_LEFT . $settings_array['upgrade_amount'] . $this->DEFAULT_SYMBOL_RIGHT;
        //
    ...
    public function insertRankDetails($rank_post_array, $commission_type, $default_currency_value)
    {
        ...
        $this->db->set('rank_incentive_reward', $rank_post_array['rank_incentive_reward']);
        $this->db->set('rank_incentive_bonus', $rank_post_array['rank_incentive_bonus']);
    ...
    public function selectRankDetails($edit_id)
    {
        ...
        //  $obj_arr['team_member_count'] = $row['team_member_count'];
            ...
            $obj_arr['rank_incentive_reward'] = $row['rank_incentive_reward'];
            $obj_arr['rank_incentive_bonus'] = $row['rank_incentive_bonus'];
            $obj_arr['binary_bonus_percentage'] = $row['binary_bonus'];
            $obj_arr['binary_monthly_cap'] = $row['binary_monthly_cap'];
        }
        return $obj_arr;
    }
    ...
    public function updateRank($rank_post_array, $commission_type, $default_currency_value)
    {
       ...
       // $this->db->set('rank_bonus', round($rank_post_array['rank_achievers_bonus'] / $default_currency_value, 8));
        ...
        $this->db->set('rank_incentive_reward', $rank_post_array['rank_incentive_reward']);
        $this->db->set('rank_incentive_bonus', $rank_post_array['rank_incentive_bonus']);
        $this->db->set('binary_bonus', $rank_post_array['binary_bonus_percentage']);
        $this->db->set('binary_monthly_cap', $rank_post_array['binary_monthly_cap']);
...
//sahla payeer config
    public function updatePayeerConfiguration($post_array)
    {
       ...
       // $this->db->set('encryption_key', $post_array['encryption_key']);
        $this->db->set('acc_no', $post_array['account']);
        $this->db->set('api_id', $post_array['api_id']);
        $this->db->set('api_key', $post_array['api_key']);
       // $result = $this->db->update('payeer_settings');
        $result = $this->db->update('payeer_details');
        return $result;
    }
    ...
    {
        $res = false;
        if (in_array($bonus_name, ['global_bonus','plan_commission_status', 'rank_commission_status', 'referal_commission_status', 'sponsor_commission_status', 'roi_commission_status', 'matching_bonus', 'pool_bonus', 'fast_start_bonus', 'performance_bonus','sales_commission','car_bonus'])) {
            $this->db->set($bonus_name, $status);
    ...
    public function updateRankConfig($post_array)
    {
        ...
        //$this->db->set('rank_expiry', $post_array['rank_expiry']);
        //$this->db->set('default_rank_id', $post_array['default_rank']);
        $this->db->set('rank_maintain_day_limit', $post_array['rank_maintain_day_limit']);
        $result = $this->db->update('rank_configuration');
    ...
    public function getFastStartBonus(){
        $this->db->select('f.*,r.rank_name');
        $this->db->from('fast_start_bonus_config as f');
        $this->db->join('rank_details as r','r.rank_id=f.rank_id','INNER');
        $query = $this->db->get();
        return $query->result_array();
    }
    public function selectfastbonusDetails($id){
        
        $this->db->select('f.*,r.rank_name');
        $this->db->from('fast_start_bonus_config as f');
        $this->db->join('rank_details as r','f.rank_id=r.rank_id','INNER');
        $this->db->where('f.rank_id', $id);
    ...
    public function updatefastBonus($rank_id, $rank_name, $level_1, $level_2, $level_3, $level_4, $level_5,$level_6,$level_7,$level_8,$level_9,$level_10) {

      
        $this->db->set('level1', $level_1);
        $this->db->set('level2', $level_2);
        $this->db->set('level3', $level_3);
        $this->db->set('level4', $level_4);
        $this->db->set('level5', $level_5);
        $this->db->set('level6', $level_6);
        $this->db->set('level7', $level_7);
        $this->db->set('level8', $level_8);
        $this->db->set('level9', $level_9);
        $this->db->set('level10', $level_10);

        $this->db->where('rank_id', $rank_id);
        $query = $this->db->update('fast_start_bonus_config');
        return $query;
    }
    ...
    public function updateCarBonusConfig($post_arr){
        
        $this->db->set('amount',$post_arr['amount']);
        $result= $this->db->update('car_bonus_configuration');
        return $result;
    }
    ...
    public function updateGlobalBonus($post_arr){
        
        $this->db->set('bonus_perc',$post_arr['bonus_percentage']);
        $result= $this->db->update('global_bonus_configuration');
        return $result;
    }
    public function  activateOrder($details){
        $this->MODULE_STATUS = $module_status = $this->login_model->trackModule();
        $this->load->model('order_model');
        $this->load->model('oc_register_model');
        $order_details['order_id']=$details[0]['order_id'];
        $order_details['user_id']=$details[0]['user_id'];
        $order_details['order_data']= $this->order_model->getOrderProductDetails($details[0]['order_id']);
    ...
    public function getPayoutGateways(){
        ...
        foreach ($query->result() as $row) {
                ...
                $details[$i]['mode'] = $row->mode;
                $details[$i]['gateway_name'] = $row->gateway_name;
                $details[$i]['payout_status'] = $row->payout_status;
                $i++;
    ...
    //stripe config  --sahla
     public function getStripeConfigurationDetails()
    {
        $details = array();
    ...
    public function updateStripeConfiguration($post_array)
    {
      
     
        $this->db->set('stripe_key', $post_array['stripe_key']);
        $this->db->set('stripe_secret', $post_array['stripe_secret']);
       // $result = $this->db->update('payeer_settings');
