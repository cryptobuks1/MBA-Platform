...
Class home_model extends inf_model {
    ...
    public function getPackageProgressData($limit = 4, $user_id = '',$from_date='') {
        $i = 0;
        $total = 0;
        $data = [];
         ...
         if ($MODULE_STATUS['opencart_status'] == "yes" || $MODULE_STATUS['opencart_status_demo'] == "yes") {
          ...
          //  $this->db->join("oc_order_history as oh", "o.order_id=oh.order_id ", "INNER");
           // $this->db->where("oh.order_status_id >=", 1);
            if ($from_date && $to_date) {
                $this->db->where('o.confirm_date >=', $from_date);
                $this->db->where('o.confirm_date <=', $to_date);
            }
            ...
            $query = $this->db->get();
            //$total_sales_order = $query->result_array()[0]['count'];echo $total_sales ."--".$total_sales_order;die;
            $total_sales_order = count($query->result_array());
    ...
    public function insertUploadDetails($file_name, $img_url, $uploaded_date,$type){
        if($type=='banner')
        {
             $arr = array(
        ...
        $res = $this->db->insert('latest_uploads', $arr);
        return $res;
    }
    
    
    
    public function update_latestupdate_banner($file_name, $img_url, $uploaded_date)
    {
        $arr = array(
            'image_name' => $file_name,
            'url' => $img_url,
        ...
        $res = $this->db->update('latest_uploads', $arr);
        return $res;
    }
    ...
    public function getAllImages(){
        ...
        foreach ($query->result_array() as $row) {
           $img_det['id']=$row['id'];
           $img_det['image_name']=$row['image_name'];
           $img_det['url']=$row['url'];
           $img_det['status']=$row['status'];
           $img_det['date']=$row['date'];
           $i++;
        }
        return $img_det;
    }
    
    public function inactivateImage($image_id){
        $result= false;
        $this->db->set('status','inactive');
        $this->db->where('id',$image_id);
         $query=$this->db->update("latest_uploads");
         if($query){
             return true;
         }
    }
    public function activateImage($image_id){
        $result= false;
    ...
    public function getRankByKeyword($keyword, $type = '') {
        $this->db->select('rank_name');
        $this->db->like('rank_name', $keyword, 'after');
        if($type != ''){
            $this->db->where('user_type !=', $type);
        }
        ...
        $response = [];
        foreach ($query->result_array() as $row) {
            $response[] = $row['rank_name'];
        }
        return $response;
    }
public function getTotalCommission_paid($date=''){
    $response = 0;
    ...
    if($date !=''){
     ...
     foreach ($query->result_array() as $row) {
         if($row['amount_payable'] != ''){
            $response = $row['amount_payable'];
         }
        }
        return $response;
}
public function getTotalActiveUserCount($date=''){
    $this->db->select('COUNT(*) as count');
    $this->db->from('ft_individual');
    ...
    $this->db->where('join_type !=', 'affiliate');
     $query = $this->db->get();
     foreach ($query->result_array() as $row) {
            $response = $row['count'];
        }
        return $response;
}public function getTotalActiveIboCount($date=''){
    $this->db->select('COUNT(*) as count');
    $this->db->from('ft_individual');
    $this->db->where('active', 'yes');
    $this->db->where('user_type !=', 'admin');
    ...
    $this->db->where('join_type', 'affiliate');
     ...
     foreach ($query->result_array() as $row) {
      ...
      return $query->row_array();
        ...
        public function editSelectedimage($new_file_name,$post_array, $id){
      //  print_r($post_array);
      //  print_r($new_file_name);die;
       if($new_file_name)
       {
            $this->db->set('image_name',$new_file_name);
       }
       else
       {
            $this->db->set('image_name',$post_array['image_name']);
       }
       
        $this->db->set('url',$post_array['url']);
      
      
        $this->db->where('id',$post_array['id']);
        $res =$this->db->update('latest_uploads');
        return $res;
    }
