...
//        }
 ...
 public function insertcalender($title,$start,$end){
    
        $this->db->set('title', substr($title, 0, 5)); 
        $this->db->set('description',$title );
        $this->db->set('start', $start);
        $this->db->set('end', $end);
        return $this->db->insert('calender_events');
    }
  public function getcalender(){
    ...
    public function deletecalender($id){
      $this->db->where('id', $id);
      return $this->db->delete('calender_events');
    }

    public function editcalender($id,$title,$start,$end){
        $this->db->set('title', $title);
        $this->db->set('start', $start);
        $this->db->set('end', $end);
        $this->db->where('id', $id);
        $res = $this->db->update('calender_events');
        return $res;
    }
    
    public function UpdateIntoUpgradeTerms($content){
        
        $this->db->set('terms_conditions',$content);
         return $res = $this->db->update('upgrade_terms_conditions');
    }
