...
//        $excel_array[2][1] = $profile_details_array["details"][0]['user_detail_pin'];
    ...
    public function getPaymentDetails($from_date, $to_date, $user_id, $type){
        
        $payment_array = $this->report_model->getPaymentDetails($from_date, $to_date, $user_id, $type);
        //   print_r($payment_array);
        //   die; 
        $count = count($payment_array);
        $excel_array[1] = array(lang('username'), lang('fullname'), lang('amount_paid'), lang('paid_date'), lang('payment_method'));
        for ($i = 2; $i <= $count + 1; $i++) {
            $j = $i - 2;
            $excel_array[$i][0] = $payment_array[$j]["username"];
            $excel_array[$i][1] = $payment_array[$j]["fullname"];
            $excel_array[$i][2] = $payment_array[$j]["amount_paid"];
            $excel_array[$i][3] = $payment_array[$j]["paid_date"];
            $excel_array[$i][4] = $payment_array[$j]["paid_type"];
    ...
    public function getBusinessWalletDetails($from_date, $to_date){
        ...
        $summary_array = $this->ewallet_model->getBusinessWalletDetails($from_date, $to_date);
          //print_r($summary_array);
        //   die; 
        $credited_amount=0;
        $debited_amount=0;
        $count = count($summary_array);
        $excel_array[1] = array(lang('transaction_category'), lang('credited_amount'), lang('debited_amount'));
        ...
        foreach($summary_array as $key=>$value){
             ...
             if($value['type']=='credit'){
                  $excel_array[$i][1] = round($value['amount']*$this->DEFAULT_CURRENCY_VALUE,8);
                  $credited_amount += $value['amount'];
             }else{
                 $excel_array[$i][1] = 0;
                 
             }
             if($value['type']=='debit'){
                  $excel_array[$i][2] = round($value['amount']*$this->DEFAULT_CURRENCY_VALUE,8);
                  $debited_amount += $value['amount'];
             }else{
                 $excel_array[$i][2] = 0;
        ...
        $excel_array[$i+1][0] = 'Total';
        $excel_array[$i+1][1] = round($credited_amount*$this->DEFAULT_CURRENCY_VALUE,8);
        $excel_array[$i+1][2] = round($debited_amount*$this->DEFAULT_CURRENCY_VALUE,8);
            $excel_array[$i+2][0] = '';
            $excel_array[$i+2][1] = "profit";
            $excel_array[$i+2][2] = round(($credited_amount-$debited_amount)*$this->DEFAULT_CURRENCY_VALUE,8);
    ...
    public function add_income($user_id,$page='',$config='',$amount_type,$filter_date){
        
        $earnings_array = $this->income_details_model->add_income($user_id, '','',$amount_type,$filter_date);
        //   print_r($earnings_array);
        //   die; 
        $count = count($earnings_array);
        $total_amount=0;
        $excel_array[1] = array( lang('from'), lang('level'), lang('amount_type'), lang('amount'));
        for ($i = 2; $i <= $count + 1; $i++) {
            $j = $i - 2;
            $excel_array[$i][0] = $earnings_array[$j]["from_user"];
            $excel_array[$i][1] = $earnings_array[$j]["user_level"];
            $excel_array[$i][2] = lang($earnings_array[$j]["amount_type"]);
            $excel_array[$i][3] = $earnings_array[$j]["amount_payable"];
           
            $total_amount+= $earnings_array[$j]["amount_payable"];
        }
        $excel_array[$i+1][0] = ' ';
        $excel_array[$i+1][1] = ' ';
       
        $excel_array[$i+1][2] = 'Amount Total';
        $excel_array[$i+1][3] = round($total_amount*$this->DEFAULT_CURRENCY_VALUE,8);
    ...
    public function getAllTransaction($user_id='', $filter_debit_credit='', $filter_category='', $filter_date=''){
        
        
        $count = $this->ewallet_model->getAllTransactionCount($user_id, $filter_debit_credit, $filter_category, $filter_date);
        //$transaction_array = $this->ewallet_model->getAllTransaction($user_id, $filter_debit_credit, $filter_category, $filter_date, $page, $config);
        $transaction_array = $this->ewallet_model->getAllTransactionDetails($user_id, $filter_debit_credit, $filter_category, $filter_date);
       // print_r($transaction_array); die;
        $transaction_data = $transaction_array['data'];
       //print_r($transaction_data); die;
        $count1 = count($transaction_data);
        
        $excel_array[1] = array( lang('amount_type'), lang('user_name'), lang('amount'),lang('date'));
        
        for ($i = 2; $i <= $count1 + 1; $i++) {
            $j = $i - 2;
            $excel_array[$i][0] = lang($transaction_data[$j]["amount_type"]);
            $excel_array[$i][1] = $transaction_data[$j]["user_name"];
            $excel_array[$i][2] = $transaction_data[$j]["amount"];
            /*if($transaction_data[$j]["ewallet_type"]=='commission' || $transaction_data[$j]["amount_type"]=='pin_purchase_refund' || ($transaction_data[$j]["ewallet_type"]=='fund_transfer' && $transaction_data[$j]["amount_type"]=='admin_credit') ||($transaction_data[$j]["ewallet_type"]=='payout')){
                $excel_array[$i][3]= lang('credit');
            }*/
            //$excel_array[$i][3] = $transaction_data[$j]["type"];
            $excel_array[$i][3] = $transaction_data[$j]["date_added"];
             }
    ...
    public function getStripeMonhtlyRecurringDetails($from_date, $to_date, $type, $user_id) {
        $commission_details_array = $this->report_model->getStripeMonhtlyRecurringDetails($type, $from_date, $to_date, $user_id);
        $count = count($commission_details_array);
        $excel_array[1] = array(lang('user_name'), lang('full_name'), lang('email'), lang('text'), lang('date'));
        for ($i = 2; $i <= $count + 1; $i++) {
            $excel_array[$i][0] = $commission_details_array[$i - 2]["user_name"];
            $excel_array[$i][1] = $commission_details_array[$i - 2]["full_name"];
            $excel_array[$i][2] = $commission_details_array[$i - 2]["email"];
            $excel_array[$i][3] = $commission_details_array[$i - 2]["text"];
            $excel_array[$i][4] = date('Y/m/d', strtotime($commission_details_array[$i - 2]["date"]));
        }
        $excel_array = $this->replaceNullFromArray($excel_array);
        return $excel_array;
