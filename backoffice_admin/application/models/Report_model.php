...
class report_model extends inf_model
{
    ...
    public function getRevenueReportDetails($module_status, $date)
    {
        ...
        /*if($date == '2020-03'){
        echo $amount->row('total_amount');die;
        }*/
        ...
        if ($this->MODULE_STATUS['opencart_status'] == 'yes') {
            ...
            $amount = $this->db->get('oc_order');
            
            $amount_credit += $amount->row('total');
        }
    ...
    public function getUserUpgradeDetails($from_date='', $to_date=''){
        ...
        $this->db->from('upgradation_history');
        ...
        foreach($query->result_array() as $row){
            ...
            $details[$i]['amount_paid'] = $row['paid_amount'];
            $details[$i]['date'] = $row['paid_date'];
            $i++;
        }//print_r($details); die;
        return $details;
     ...
     public function getPaymentDetails($from_date='', $to_date='', $user_id='', $type = ''){
        ...
        if($type != ''){
            if($type=="Manual Stripe Recurring"){
                $this->db->where("s.type", $type);
            }
            elseif($type=="Stripe Recurring"){
                
                $this->db->where("s.type", $type);
            }
            else{
                $this->db->like('s.type', "Referal Count");
            }
        ...
        foreach($query->result_array() as $row){
            ...
            $details[$i]['paid_date'] = $row['date'];
            $details[$i]['amount_paid'] = $this->cron_model->getRecurringAmount();
             $i++;
        }//print_r($details); die;
        return $details;
    }
    ...
    function getStripeMonhtlyRecurringDetails($type, $from_date = '', $to_date = '', $user_id = '')
    {
        $date = date("Y-m-d H:i:s");
        ...
        $this->db->from('stripe_monhtly_recurring_history');
        // code to remove
        $this->db->where_not_in("id", array(1562, 1557, 1552, 1544));
        // code to remove
        $this->db->group_by('user_id');
        ...
        if ($type != '') {
            if (in_array('pending', $type) && !in_array('active', $type)) {
                if (!$user_id) {
                    $this->db->where('type', 'failed');
                }
            }
            if (!in_array('pending', $type) && in_array('active', $type)) {
                $this->db->where('type!=', 'failed');
        ...
        if ($type != '') {
            if (in_array('pending', $type)) {
                $this->db->select("id AS user_id, 'Subscription Period Exceeded' AS type, 'subscription' AS status, 'subscription' AS amount, subs_end_date AS date");
                $this->db->from('ft_individual');
                $this->db->where("subs_end_date <", date("Y-m-d H:i:s"));
                if ($user_id != '') {
                    $this->db->where("id", $user_id);
                }
                if ($result) {
                    $this->db->where_not_in("id", array_column($result, 'user_id'));
                }
                if ($from_date != '') {
                    $this->db->where("subs_end_date >=", $from_date);
                }
                if ($to_date != '') {
                    $this->db->where("subs_end_date <=", $to_date);
                }
                $query = $this->db->get();
                $result1 = $query->result_array(); //echo "<pre>";print_r($query->result_array());die;
                $num_rows = $query->num_rows();
                if ($num_rows > 0) {
        ...
        if (count($result) > 0) {
            foreach ($result as $row) {
                $user_status = $this->validation_model->getUserStatus($row['user_id']);
                if($user_status=='active'){
                    $details[$i]['user_name'] = $this->validation_model->IdToUserName($row['user_id']);
                    $details[$i]['full_name'] = $this->validation_model->getFullName($row['user_id']);
                    $details[$i]['email'] = $this->validation_model->getUserEmailId($row['user_id']);
                    $details[$i]['type'] = $row['type'];
                    if(!isset($row['text'])){
                        $details[$i]['text'] = $row['type'];
                    }else{
                        $details[$i]['text'] = $row['text'];
                    }
                    // $details[$i]['status'] = $row['status'];
                    $details[$i]['status'] = '';
                    // $details[$i]['amount'] = $row['amount']/100;
                    $details[$i]['amount'] = '';
                    $details[$i]['date'] = $row['date'];
                    $i = $i + 1;
                }
...
array_multisort(array_map('strtotime',array_column($details,'date')),
                SORT_DESC, 
                $details);//echo "<pre>";print_r($details);die;
        return $details;
    }
}
