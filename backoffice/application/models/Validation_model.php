...
Class validation_model extends inf_model {
    ...
    public function sentPassword($user_id, $password, $user_name) {
     /*   $this->load->model('mail_model');
        ...
        $mailBodyDetails = <html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
...
public function getUserDetails($user_id, $type = 'user') {
    ...
    public function getSettings() {
        ...
        foreach ($res->result_array() as $row) {
            ...
            $obj_arr["stripe_key"] = $row['stripe_key'];
            $obj_arr["stripe_secret"] = $row['stripe_secret'];
        }
...
//        $this->db->where('view_status', 'yes');
    ...
    public function getJoinType($user_id) {
    $user_type = "";
    $this->db->select('join_type');
    $this->db->where('id', $user_id);
    $res = $this->db->get('ft_individual');
    foreach ($res->result_array() as $row) {
        $user_type = $row['join_type'];
    }
    return $user_type;
}

    public function getTotalLeftLeg($user_id){
    
        $this->db->select_sum('left_leg');
        $this->db->where('user_id', $user_id);
        
        $query = $this->db->get('leg_amount');
        foreach ($query->result() as $row) {
            $left_volume = $row->left_leg;
        }return $left_volume;
    }
    
    public function getTotalRightLeg($user_id){
    
        $this->db->select_sum('right_leg');
        $this->db->where('user_id', $user_id);
        
        $query = $this->db->get('leg_amount');
        foreach ($query->result() as $row) {
            $right_volume = $row->right_leg;
        }return $right_volume;
    }
    //by neenu
     public function getFtData($user_id, $field_name = "") {
        $data = null;
        if ($field_name != "") {
            $this->db->select($field_name);
  ...
  public function getDownlineUsersCount($left_no, $right_no, $type, $pv) {
    ...
    public function getPreviousStripeDetails($user_id){
        $row=array();
        $customer_id = $this->getOcCustomerId($user_id);
        $this->db->select('ort.reference as ref,or.recurring_duration as duration, or.recurring_price as amount, o.customer_id cust_id, or.product_id, or.product_quantity, ort.order_recurring_id as order_recurring_id,ort.amount rec_tot_amount, o.order_id');
    ...
    public function addMonthlyRecurringHistory($row,$text,$status){
        ...
        public function add_strip_paymentRecurringHistory($row,$text,$status,$payment_status){
        $today = date('Y-m-d');
        if($status=='success'){
            $end_date = date('Y-m-d', strtotime("$today + 30 days")); 
            $this->db->set('nxt_reccuring_date',$end_date);
        ...
        $this->db->set('recurr_order_id',$row['order_recurring_id']);
        return $this->db->insert('user_upgrade_strip_history');
    }
...
public function isUpgradedUser($user_id){
    
        $this->db->select('user_id');
        $this->db->where('user_id',$user_id);
        $count = $this->db->count_all_results('upgradation_history');
        return $count;
    }
    ...
    public function getProductDescriptionFromProdID($product_id) {
        $name = '';
        $this->db->select("oc_product_description.name");
        $this->db->from("oc_product");
        $this->db->join("oc_product_description", "oc_product.product_id=oc_product_description.product_id");
        $this->db->where("oc_product.package_id", $product_id);
        $query = $this->db->get();
        foreach ($query->result() as $row) {
            $name = $row->name;
        }
