...
class ewallet_model extends inf_model {
    ...
    public function getBusinessWalletDetails($type = '')
    {
        $wallet_details = [];
        ...
        if ($this->MODULE_STATUS['opencart_status'] == 'yes') {
            ...
            $purchase_amount = ($query->row_array()['total'] ?? 0) - $joining_fee;
...
//Purchase wallet starts
    ...
    public function insertUpgradeHistory($user_id='', $req_amount='', $date, $paid_type=''){
        
        $this->db->set('user_id', $user_id);
        $this->db->set('paid_amount', $req_amount);
        $this->db->set('paid_date', $date);
        $this->db->set('paid_type', $paid_type);
        $rs = $this->db->insert('upgradation_history');
        if($paid_type == 'by_ewallet'){
        $this->validation_model->addEwalletHistory($user_id, $user_id, $payment_id='', 'customer_upgrade', $req_amount, 'user_debit', 'debit', $transaction_id='',$transaction_concept='', $trans_fee='');
        }
        return $rs;
    }
    
    public function upgradeJoinType($user_id = ''){
        
        $this->db->set('join_type','affiliate');
    ...
    public function getMonthlyFee(){
        ...
        foreach($query->result_array() as $row){
            ...
            $monthly_fee = $row['monthly_subs_fee'];
        }
        return $monthly_fee;
    }
    
    public function getSubscriptionEndDate($user_id=''){
       // $date= "2020-05-01 11:13:50";
    ...
    public function updateSubscriptionEndDate($user_id='', $month=''){
        
        $date=date('Y-m-d H:i:s');
          
        $endDate= date('Y-m-d H:i:s', strtotime($date ." +".$month."month"));
        
        $this->db->set('subs_end_date',$endDate);
        $this->db->where('id',$user_id);
        $rs=$this->db->update('ft_individual');
       
        return $rs;
    }
    public function insertPaymentDetails($userid='', $trans_amt='', $transaction_id=''){
      
        $date = date('Y-m-d H:i:s');
        $previous_end_date=$this->ewallet_model->getSubscriptionEndDate($userid); 
        
             $data = array(
                'paid_user_id' => $userid,
                'paid_amount' => $trans_amt,
                'paid_date' => $date,
                'paid_type' => 'manual_payment',
                'transaction_id' => $transaction_id,
                'paid_status'=>'yes',
           ...
           $this->addSubscriptionHistory($userid, $trans_amt,'manual_payment',$payment_id,$transaction_id,'ewallet', $previous_end_date);
           $this->validation_model->addEwalletHistory( $userid,$userid, $payment_id, 'monthly_payment', $trans_amt, 'user_debit', 'debit', $transaction_id,$transaction_concept='', $trans_fee='');
        //  $this->validation_model->addEwalletHistory($to_user_id, $from_user_id, $ewallet_id, 'fund_transfer', $trans_amount, $amount_type, ($amount_type == 'admin_debit') ? 'debit' : 'credit', $transaction_id, $transaction_concept, $trans_fee);
         }
    
         public function addSubscriptionHistory($userid, $trans_amt, $paid_type, $payment_id,$transaction_id,$payment_method, $previous_end_date ) {
           
            $date = date('Y-m-d H:i:s');
         
             
             
             $monthly_payment= $this->getMonthlyFee();
             
             
             $this->db->set('payment_id', $payment_id);
             
             $this->db->set('user_id', $userid);
             $this->db->set('amount', $trans_amt);
             $this->db->set('paid_date', $date);
             $this->db->set('transaction_id', $transaction_id);
             $this->db->set('paid_type', $paid_type);
             $this->db->set('payment_method', $payment_method);
             $this->db->set('previous_subscription_end_date', $previous_end_date);
             
             $this->db->set('current_subscription',$monthly_payment);
             
             $rs = $this->db->insert('subscription_payment_detail');
             
             return $rs;
        }
    public function addSubscriptionDetails($curl_data, $curl_type, $recurring_id, $order_id, $item, $subscription,$oc_cust_id){
        $this->db->set('stripe_customer_id',$recurring_id);
        $this->db->set('order_id',$order_id);
        $this->db->set('date_added',date('Y-m-d H:i:s'));
        $this->db->set('status',6);
        $this->db->set('product_id',$item['product_id']);
        $this->db->set('product_name',$item['name']);
        $this->db->set('product_quantity',$item['quantity']);
        $this->db->set('recurring_id',$item['recurring']['recurring_id']);
        $this->db->set('recurring_name',$item['recurring']['name']);
        $this->db->set('recurring_description','subscription_added');
        $this->db->set('recurring_frequency',$item['recurring']['frequency']);
        $this->db->set('recurring_price',$item['recurring']['price']);
        $this->db->set('recurring_cycle',$item['recurring']['cycle']);
        $this->db->set('trial_cycle',$item['recurring']['trial_cycle']);
        $this->db->set('trial_duration',$item['recurring']['trial_duration']);
        $this->db->set('trial_price',$item['recurring']['trial_price']);
        $this->db->set('reference',$subscription);
        $this->db->insert('oc_order_recurring');
        //$this->db->query("INSERT INTO `23326_oc_order_recurring` SET `stripe_customer_id` = '" . $recurring_id . "',`order_id` = '" . (int) $order_id . "', `date_added` = NOW(), `status` = 6, `product_id` = '" . (int) $item['product_id'] . "', `product_name` = '" . $this->db->escape($item['name']) . "', `product_quantity` = '" . $this->db->escape($item['quantity']) . "', `recurring_id` = '" . (int) $item['recurring']['recurring_id'] . "', `recurring_name` = '" . $this->db->escape($item['recurring']['name']) . "', `recurring_description` = 'subscription_added', `recurring_frequency` = '" . $this->db->escape($item['recurring']['frequency']) . "', `recurring_cycle` = '" . (int) $item['recurring']['cycle'] . "', `recurring_duration` = '" . (int) $item['recurring']['duration'] . "', `recurring_price` = '" . (float) $item['recurring']['price'] . "', `trial` = '" . (int) $item['recurring']['trial'] . "', `trial_frequency` = '" . $this->db->escape($item['recurring']['trial_frequency']) . "', `trial_cycle` = '" . (int) $item['recurring']['trial_cycle'] . "', `trial_duration` = '" . (int) $item['recurring']['trial_duration'] . "', `trial_price` = '" . (float) $item['recurring']['trial_price'] . "', `reference` = '" . $subscription . "'");
        //print_r($item['recurring']['frequency']);die;

        $today = strtotime(date("Y-m-d"));
        $nxt_reccuring = date('Y-m-d', strtotime('+1 month', $today));
        $this->db->set('order_recurring_id',$this->db->insert_id());
        $this->db->set('amount',$item['recurring']['price']);
        $this->db->set('type','recurring added from manual backoffice');
        $this->db->set('date_added',date("Y-m-d H:i:s"));
        $this->db->set('nxt_reccuring_date',$nxt_reccuring);
        $this->db->set('reference',$recurring_id); 
        $this->db->set('oc_cust_id',$oc_cust_id);
        $this->db->insert('oc_order_recurring_transaction');
        //$this->db->query("INSERT INTO `23326_oc_order_recurring_transaction` SET `order_recurring_id` = '" . $this->db->getLastId() . "',`reference` = '" . $recurring_id . "',`nxt_reccuring_date` = '" . $nxt_reccuring . "', `date_added` = NOW(), `type` = 'reccuring_added', `amount` = '" . $this->db->escape($item['quantity']) * $item['recurring']['price'] . "'");
    
        
    }
    
    public function getUpgradeTermsConditions($lang_id = ''){
        
        $this->db->select('terms_conditions');
        
        $this->db->from('upgrade_terms_conditions');
    ...
    public function getUpgradationNote(){
        $text='';
        $this->db->select('content');
        $this->db->where('subject','upgrade_note');
    ...
    public function updateFtPack($pack='',$user_id=''){
        $this->db->set('product_id','pck2');
        $this->db->where('id',$user_id);
        $rs=$this->db->update('ft_individual');
        return $rs;
    }
   public function getRefFromCustomerId($oc_cust_id){
        $text='';
        $this->db->select('reference');
        $this->db->where('oc_cust_id',$oc_cust_id);
        $this->db->from('oc_order_recurring_transaction');
        $query = $this->db->get();
         foreach($query->result() as $row){
            $text = $row->reference;
        }
