...
class payout_model extends inf_model {
    ...
    public function insertPayoutReleaseRequest($user_id, $payout_amount, $request_date, $status = 'pending', $payout_method) {
    ...
    public function getBitcoinDetails($user_id) {
    ...
    public function getPaymentMethodDetails($label) {

        $this->db->select('gateway_name');
        $this->db->where('payout_status', "yes");
        $this->db->where('gateway_name !=', $label);
        $this->db->order_by('payout_sort_order', "ASC");
        $this->db->from('payment_gateway_config');
        $res = $this->db->get();
    ...
    public function getDetails($label1, $label2){
        
        $this->db->select('gateway_name');
        $this->db->where('payout_status', "yes");
        $this->db->where('gateway_name !=', $label1);
        $this->db->where('gateway_name !=', $label2);
        $this->db->order_by('payout_sort_order', "ASC");
        $this->db->from('payment_gateway_config');
        $res = $this->db->get();
        return $res->result_array();
